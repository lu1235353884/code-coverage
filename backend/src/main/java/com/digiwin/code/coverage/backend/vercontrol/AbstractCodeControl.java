...
@Data
@Slf4j
public abstract class AbstractCodeControl {

    protected CodeDiffDto codeDiffDto;
     ...
     * @return
     */
    public DiffInfo getDiffCodeHandler(CodeDiffDto codeDiffDto) {
        this.codeDiffDto = codeDiffDto;
        getDiffCode();
        List<DiffClassInfoResult> diffCodeMethods = getDiffCodeMethods();
        return DiffInfo.builder()
                .baseLocalPath(codeDiffDto.getBaseLocalPath())
                .compareLocalPath(codeDiffDto.getCompareLocalPath())
     ...
     * 下载代码
     *
     * @param codeInvokeDto 方法调用dto
     * @return {@link String}
     */
    public abstract String downloadCode(CodeInvokeDto codeInvokeDto);


    public abstract List<String> getRemoteBranchs(String appId);
     ...
     * @description: 获取差异类
     */
    public abstract void getDiffCode();
     ...
     */
    public abstract String getLocalComparePath(String filePackage);
     ...
     */
    public abstract String getLocalBasePath(String filePackage);
    ...
    public List<DiffClassInfoResult> getDiffCodeMethods() {
        if (CollectionUtils.isEmpty(codeDiffDto.getDiffClasses())) {
            return null;
        }
        LoggerUtil.info(log, "需要对比的差异类数", codeDiffDto.getDiffClasses().size());
        List<CompletableFuture<DiffClassInfoResult>> priceFuture = codeDiffDto.getDiffClasses().stream()
                .map(item -> getClassMethods(getLocalBasePath(item.getNewPath()), getLocalComparePath(item.getNewPath()), item))
